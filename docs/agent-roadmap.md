...
| C7 | ~~`find_correlation_candidates` / `apply_correlation`~~ **Done** | L | Exposes the existing "Correlation Studio" pipeline (`CorrelationEngine`/`CorrelationInjector`, previously only reachable via its own `CorrelationReviewDialog` menu item) to the agent as a find→review→apply pair, per the user's decision (vs. a single no-review `auto_correlate` tool). `find_correlation_candidates` runs `CorrelationEngine.runAndCorrelate()` (a forced 1-thread/1-loop probe, unlike C6's as-configured run - needed to unambiguously trace which later sampler reuses a value) and reports each finding with a 1-based candidate id; non-destructive, does not touch the plan. `apply_correlation` takes `candidate_ids` (or `apply_all`), marks just those `CorrelationCandidate`s `APPROVED`, and delegates to `CorrelationInjector.apply` (adds a Regex/JSON extractor to the source sampler, rewrites every reusing sampler's matching value to `${variable}`) - registered in `JMeterAgent.DESTRUCTIVE_TOOLS` for the confirmation dialog, same as `delete_element`/`move_element`/`open_plan`. New `agent.correlation` package: `CorrelationCandidateStore` (static, since `AgentToolRegistry.createDefault()` builds fresh handler instances per agent turn - find and a later apply need to share state across turns) + `CorrelationCandidateFinder`/`CorrelationApplier` seams. |
...
| #  | Task                                                      | Effort | Notes                                                                  |
|----|-----------------------------------------------------------|--------|------------------------------------------------------------------------|
| E1 | ~~Dev menu items for `toggle_element` / `move_element`~~ **Done** | S | Added `ToggleElementDevMenuItem`/`ToggleElementDevRunner` and `MoveElementDevMenuItem`/`MoveElementDevRunner`, mirroring the existing `delete_element` dev pattern. Registered in `AiMenuCreator` alongside the other dev items. Each has full unit tests with fake seams (7 toggle tests, 6 move tests). |
| E2 | ~~User-facing docs: enabling agent mode, settings, examples~~ **Done** | S | Added comprehensive Agent Mode section to `README.md` (enabling, settings table, 18-tool reference grouped by category, how-it-works flow, safety notes, examples table, dev menu items). Added agent properties to `jmeter-ai-sample.properties`. |
...
Phases A, B, and C are done. Phase D (platform & providers) is deferred. E1 (dev
menu items) and E2 (user-facing docs) are done. Next up: **E3** (lightweight
