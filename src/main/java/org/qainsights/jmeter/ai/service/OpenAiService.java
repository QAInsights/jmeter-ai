...
public class OpenAiService implements AiService {
    ...
    public OpenAiService() {
        ...
        String baseUrl = AiConfig.getProperty("openai.base.url", Constants.DEFAULT_OPENAI_BASE_URL);
        ...
        OpenAIOkHttpClient.Builder clientBuilder = new OpenAIOkHttpClient.Builder().apiKey(apiKey);
        if (baseUrl != null && !baseUrl.trim().isEmpty()) {
            clientBuilder.baseUrl(baseUrl);
        }
        this.client = clientBuilder.build();
        log.info("Initialized OpenAI client with baseUrl: {}", baseUrl);
    ...
    public String generateResponse(List<String> conversation) {
        try {
            ...
            long actualMaxTokens = maxTokens;
            if (!supportsCustomTemperature && actualMaxTokens < 4096) {
                actualMaxTokens = 4096;
                log.info("Automatically increased maxCompletionTokens to {} for reasoning model {}", actualMaxTokens, currentModelId);
            }

            // Create a fresh builder for parameters following the working example
            ChatCompletionCreateParams.Builder paramsBuilder = ChatCompletionCreateParams.builder()
                    .maxCompletionTokens(actualMaxTokens)
            ...
            if (responseContent == null || responseContent.trim().isEmpty()) {
                responseContent = "No content was generated by the AI model. This can happen if the token limit (max completion tokens) was reached during the model's reasoning phase. Please try increasing the 'openai.max.tokens' property in your JMeter configuration.";
                log.warn("AI model response content was empty or blank (possibly exhausted reasoning token budget)");
            }
